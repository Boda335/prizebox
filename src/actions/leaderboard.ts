 ...
 */
export function getLeaderboard(manager: GiveawaysManager, type: 'entries' | 'wins' = 'entries', top = 10) {
  ...
  for (const guildId in allStats) {
    ...
    for (const userId in guildStats) {
      ...
      if (!stats[userId]) {
        ...
        const found = manager.giveaways.flatMap(g => g.data.participants).find(p => p.id === userId);
 ...
 */
export async function sendLeaderboard(manager: GiveawaysManager, channel: TextChannel, type: 'entries' | 'wins' = 'entries', top = 10) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor('#00AAFF')
    .setDescription(lb.map(u => `**#${u.rank}** <@${u.id}> — ${type === 'entries' ? `${u.entries} entries` : `${u.wins} wins`}`).join('\n'))
