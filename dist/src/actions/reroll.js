...
exports.rerollGiveaway = rerollGiveaway;
/**
 ...
 */
async function rerollGiveaway(manager, messageId, winnerCount) {
    // Find the giveaway
    ...
    if (!giveaway.data.ended)
        throw new Error('Giveaway not ended yet');
    // Get the channel
    ...
    await channel.send(manager.messages.winMessage
        .replace('{winners}', winners.map(w => `<@${w.id}>`).join(', '))
        .replace('{this.prize}', giveaway.data.prize)
        .replace('{this.messageURL}', msg.url));
    manager.emit('giveawayRerolled', winners, giveaway);
    return winners;
}
